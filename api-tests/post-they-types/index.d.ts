...
interface Test {
  /**
   * Write test specifications. Naming the test accurately ensures the rest of the
   * script is not blocked even if there are errors inside the function.
   */
  (testName: string, specFunction: Function): void;
  /** Get the total number of tests from a specific location. */
  index(): number;
  /** Skip a test case. */
